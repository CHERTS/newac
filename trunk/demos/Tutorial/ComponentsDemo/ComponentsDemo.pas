...
(* Class: TDemoWaveIn 
  ...
  public
    ...
    *)
    ...
    (* Procedure: GetDataInternal
      The GetDataInternal method is used by NewAC to retrieve decoded data. You should pass the number of bytes you want to read in the  Bytes parameter. This method returns the pointer to the data buffre in the Buffer variable parameter and the number of bytes actually read in the buffer in the Bytes paramter. The buffer pointer must be provided by THIS method and not by the caller. The pointer returned by GetData should be valid until the next call to the method. When the input data end is reached this method should return nil in the Buffer and 0 in the Bytes parameter. The GetData method can return less bytes than it is asked to and it should not be considered as an end of file indication. 
    ...
    (* Procedure: SeekInternal 
      Performs the file seeking. The new position is set in samples, not in bytes, relatively to the beginning of the file. Seek should return True if the input is seekable and False otherwise. This method implementation is optional. If your input component doesn't support seeking You can implement in like this:
  ...
  > end;
      ...
      As you can see the argument of SeekInternal is pased by reference. If the caller requests to move to a sample that doesn't exists, (that's beyond the end of file) you can set her to the correct sample number. This behavoir s optional, as you can simply return False.
